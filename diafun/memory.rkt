...
(define-type Dia-RAM-Snapshot (-> Dia-Reversed-Variables Symbol String [#:layout Dia-RAM-Variable-Layout] Dia:RAM))
(define-type Dia-RAM-Snapshots (Immutable-HashTable Symbol (Listof Dia:RAM)))
...
(define dia-ram-snapshots
  (lambda [#:main [cfun-name : Symbol (default-ram-entry)] #:c-argv [cargv : (Listof Any) null]
    ...
    (parameterize ([default-c-source (and (path? modpath) modpath)])
      (let wsl ([snapshots : (Immutable-HashTable Symbol (Listof Dia:RAM)) (hasheq)])
        ...
        (cond [(c-ram-segment-snapshot? message)
               (wsl (hash-set snapshots (car message)
                              (cons (take-snapshot (cddr message) (car message) (cadr message) #:layout layout)
                                    (hash-ref snapshots (car message) (inst list Dia:RAM)))))]
              [(not (string? message))
               (for/hasheq : (Immutable-HashTable Symbol (Listof Dia:RAM)) ([(segment geos) (in-hash snapshots)])
