...
(define dia-memory-snapshot
  (lambda [#:id [id : (Option Symbol) #false] #:gapsize [gapsize : Real 8.0] #:reverse-address? [reverse? : Boolean #true]
    ...
    (parameterize ([default-dia-node-base-style make-memory-location-fallback-style])
      (let make-placeholder ([vars : (Listof (U C-Variable C-Pad)) variables]
              ...
              (cond [(not style) (make-placeholder rest swor)]
                    [(c-padding? self)
                     (if (or no-padding?)
                         ...
                         (make-placeholder rest (append swor (dia-padding-raw style address raw p-radix padding-limit))))]
                    [(c-variable? self)
                     (define rows : (Listof (List Geo Geo))
                       (if (and combine-datum?)
                           (dia-variable-datum #:segment (c-variable-segment self) #:rendering-segment segment
                                               style vname address (unbox (c-variable-datum self)) fx-radix)
                           (dia-variable-raw #:segment (c-variable-segment self) #:rendering-segment segment
                                             style vname address raw rd-radix)))
...
(define dia-memory-snapshots : (->* (Module-Path)
                                    (Dia-Memory-Snapshot #:main Symbol #:c-argv (Listof Any) #:watch_variable Symbol #:take_snapshot Symbol
                                                         #:optimize? Boolean #:lookahead-size Byte #:lookbehind-size Byte #:body-limit Index)
                                    Dia-Memory-Snapshots)
  (lambda [#:main [cfun-name 'main] #:c-argv [cargv null] #:watch_variable [watch 'watch_variable] #:take_snapshot [take 'take_snapshot]
    ...
    (place-channel-put ghostcat (cons watch take))
...
(define dia-memory-snapshots->table : (->* (Dia-Memory-Snapshots)
                                           (#:id (Option Symbol) #:segments (Listof Symbol) #:reverse-address? Boolean
                                            ...
                                            #:gapsize Real #:row-alignment Geo-Pin-Anchor)
                                           Geo)
  (lambda [#:id [id #false] #:segments [specific-segments null]
           ...
           #:gapsize [gapsize 16.0] #:row-alignment [ralign 'ct] #:reverse-address? [reverse? : Boolean #true]
           all-snapshots]
    ...
    (if (not headers)
        (geo-table* #:id id
                    rows
                    '(rc) (list ralign) gapsize gapsize)
        (geo-table* #:id id
                    (cons (if (and no-row-name?) headers (cons #false headers)) rows)
                    '(cc rc) (list 'cc ralign) gapsize gapsize))))
