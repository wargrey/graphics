  ...
  (lambda [code style indices]
    ...
    (define desc : Geo
      (assert (if (hash-has-key? non-printable-chars code)
                  (expr-slot-text-term (format "[~a]" (cadr (hash-ref non-printable-chars code))) style)
                  (expr-slot-text-term (string ch) style))))
    (define self : Geo (geo-text code #:color 'DimGrey))
    (define dh (geo-height desc))

    (geo-rb-superimpose
     (geo-cc-superimpose (geo-rectangle (* dh 4.0) (* dh 3.0) #:stroke #false #:fill #false)
                         desc)
     self)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define ascii
  ((inst $array Byte) #:ncols 8 #:gap 2.0
