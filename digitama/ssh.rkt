...
(define-type/consts sm : SSH-Message-Type of Byte as packet:ssh
  ...
  [SSH_MSG_IGNORE                     2 ([data : String])] ; one way for thwarting trffic analysis [rfc4251, section-9.3.9]
  ...
  (class object% (super-new)
        ...
        (lambda [tcpout packet continuation]
          ...
          (define-values (offset total-length payload-length padding-length)
            (match continuation
              ...
              [_ (let ([payload : Bytes (for/fold ([payload : Bytes (bytes msgid)])
                   ...
                   ;;; TODO: https://tools.ietf.org/html/rfc4253#section-6.2
                   ;   However there is no plan to implement this feature
                   ...
                   (for ([i (in-range padding-length)])
                     (bytes-set! send-buffer (+ 5 payload-length i) (bytes-ref payload (random payload-length))))
                ...
                [else (let ([sent (+ offset resent)])
                        (if (< sent total-length)
                            (throw [exn:ssh:again (list sent total-length padding-length)]
                                   "sending ~a, ~a bytes, ~a% done" id sent (~% sent total-length))
                            (ssh-log 'info #:urgent (vector packet total-length)
                                     "sent ~a, ~a bytes in total (+ 4 1 ~a ~a ~a)"
                                     id total-length payload-length padding-length message-authsize)))])))

      (define transport-recv-packet : (-> Input-Port Natural SSH-Channels Nothing)
        (lambda [tcpin start /dev/channel/usrout]
          ...
          (cond [(eof-object? received)
                ...
                [else (let extract-packet ([bufstart : Natural 0]
                        ...
                        (define msgid : Byte (bytes-ref recv-buffer 5))
                        (define id : (Option SSH-Message-Type) ($%sm msgid))
                        (when (> total-length packet-upsize)
                          (throw [exn:ssh:eof 'SSH_DISCONNECT_HOST_NOT_ALLOWED_TO_CONNECT]
                                 "~a bytes! packet[~a] is corrupt in length!" total-length (or id msgid)))
                        (when (> total-length bufused)
                          (throw [exn:ssh:again bufused] "receiving ~a, ~a bytes, ~a% done" (or id msgid) bufused (~% bufused total-length)))
                        (with-handlers ([exn? (lambda [[e : exn]] (ssh-log 'debug #:urgent e (exn-message e)))])
                          (define payloads : (Listof Any)
                            (let extract : (Listof Any) ([sdaolyap : (Listof Any) null] [pos : Natural 6] [types (if id ($:sm id) null)])
                              (match types
                                [(? null?)
                                 (cond [(= pos max-position) (reverse sdaolyap)]
                                       [else (throw exn:ssh "ignored ~a: corrupt size (~a/~a)" id pos max-position)])]
                                [(list 'Bytes) ; this packet has special datum.
                                 (cond [(<= pos max-position) (reverse (cons (subbytes recv-buffer pos max-position) sdaolyap))]
                                       [else (throw exn:ssh "ignored ~a: corrupt size (~a > ~a)" id pos max-position)])]
                                [(list 'Byte rest ...)
                                ...
                                [(list 'String rest ...)
                                 (let ([strsize : UInt32 (ssh-bytes->uint32 recv-buffer #:offset pos)])
                                   (when (> strsize payload-length)
                                     (throw exn:ssh "ignored ~a: corrupt string size (~a > ~a)" id strsize payload-length))
                                ...
                                [(list 'Symbol rest ...)
                                 (let ([symsize : UInt32 (ssh-bytes->uint32 recv-buffer #:offset pos)])
                                   (when (> symsize payload-length)
                                     (throw exn:ssh "ignored ~a: corrupt symbol size (~a > ~a)" id symsize payload-length))
                                ...
                                [(list 'MPInteger rest ...)
                                 (let ([mpsize : UInt32 (ssh-bytes->uint32 recv-buffer #:offset pos)])
                                   (when (> mpsize payload-length)
                                     (throw exn:ssh "ignored ~a: corrupt mpint size (~a > ~a)" id mpsize payload-length))
                                ...
                                [(list (list 'Listof _) rest ...)
                                 (let ([namesize : UInt32 (ssh-bytes->uint32 recv-buffer #:offset pos)])
                                   (when (> namesize payload-length)
                                     (throw exn:ssh "ignored ~a: corrupt namelist size (~a > ~a)" id namesize payload-length))
                          ...
                          (ssh-log 'info #:urgent (vector total-length ($*sm (or id msgid) payloads))
                                   "received ~a, ~a bytes in total (+ 4 1 ~a ~a ~a)"
                                   (or id msgid) total-length payload-length padding-length message-authsize))
                        (extract-packet total-length (max (- bufused total-length) 0)))])))

      (define transport-handle-packet : (-> packet:ssh Any)
        (lambda [packet]
          (match packet
            [(ssh:msg:kexinit cookie kex publickey
                              cipher/ctos cipher/stoc mac/ctos mac/stoc compression/ctos compression/stoc lang/ctos lang/stoc
                              guessed-key-followed? reserved/zero)
             (ssh-log 'debug #:urgent 'SSH_MSG_KEXINIT "peer proposed hostkey types: ~a" publickey)
             (ssh-log 'debug #:urgent 'SSH_MSG_KEXINIT "peer proposed key exchange algorithms: ~a" kex)
             ...
             (when guessed-key-followed? ; https://tools.ietf.org/html/rfc4253#section-7.1
               (ssh-log 'error #:urgent 'SSH_MSG_KEXINIT "peer has guessed key exchange packet followed, we have to handle it."))
             (box (ssh:msg:kexinit (call-with-input-string (number->string (current-inexact-milliseconds)) md5-bytes)
                                   ssh-algorithms/kex ssh-algorithms/publickey ssh-algorithms/cipher ssh-algorithms/cipher
                                   ssh-algorithms/mac ssh-algorithms/mac ssh-algorithms/compression ssh-algorithms/compression
                                   null null #false 0))])))
               ...
               (match (vector level message urgent)
                 [(vector 'info message (vector (? exact-positive-integer? traffic) (? packet:ssh? packet)))
                  ...
                  (match (with-handlers ([exn? values]) (transport-handle-packet packet))
                    [(box (and kexback (ssh:msg:kexinit cookie kex publickey
                                                        cipher/ctos cipher/stoc mac/ctos mac/stoc compression/ctos compression/stoc lang/ctos lang/stoc
                                                        guessed-key-followed? reserved/zero)))
                     (thread-rewind-receive (list (cons kexback #false)))
                     (dispatch /dev/tcpin 0 /dev/tcpout $out packet-evt)]
                    [(box (? packet:ssh? feedback))
                     (thread-rewind-receive (list (cons feedback #false)))
                     (dispatch /dev/tcpin 0 /dev/tcpout $out packet-evt)]
                    [(? exn:ssh:eof? e)  
                     (ssh-sendmail 'recvback e)
                     (dispatch /dev/tcpin eof /dev/tcpout eof /dev/pkt)]
                    [notification-as-always
                     (ssh-sendmail 'recvback notification-as-always)
                     (dispatch /dev/tcpin 0 /dev/tcpout $out packet-evt)])]
                 [(vector 'info message (vector (? packet:ssh? packet) (? exact-positive-integer? traffic)))
                  (on-debug 'info message (cons 'sent traffic) topic)
                  (if (false? (ssh:msg:disconnect? packet))
                      (dispatch /dev/tcpin $in /dev/tcpout $out packet-evt)
                      (with-handlers ([exn? void])
                        #| Destructor |#
                        ; TODO: check if there are more log events
                        (on-debug 'info (ssh:msg:disconnect-description packet) 'SSH_MSG_DISCONNECT topic)
                        (when (tcp-port? /dev/tcpin) (tcp-abandon-port /dev/tcpin))
                        (when (tcp-port? /dev/tcpout) (tcp-abandon-port /dev/tcpout))))]
                 [(vector 'info message (vector our-identification (? input-port? tcpin) (? output-port? tcpout)))
              ...
              [(? input-port? tcpin)
               (match (with-handlers ([exn? values])
                        (cond [(exact-nonnegative-integer? $in)
                               (transport-recv-packet tcpin $in channels)]
                 ...
                 [(? exn? e) ; (read-line) would cause another exception rather than exn:ssh:eof.
                 ...
                 [(and (? string?) (pregexp #px"^SSH-([^-]+)-(\\S+)(.*)$" (list identification\r protocol software comment\r)))
                  ...
                  (ssh-sendmail 'recvback (list (string-trim identification\r) protocol software (string-trim (~a comment\r))))
                  (sync/timeout 0.016 never-evt) ; make sure client handles identification before receiving KEXINIT in localhost
              ...
              [(cons (? packet:ssh? packet) (? boolean? block?))
               (match (unless (eof-object? $out)
                 ...
                 [(and (exn:ssh:again _ _ (and (list sent total padded) continuation)) eagain)
                  ...
                  (dispatch /dev/tcpin (if (ssh:msg:disconnect? packet) eof $in) ; Manually make TCP no longer reading
                            /dev/tcpout continuation (make-pullout-evt /dev/tcpout))]
                 [status ; TODO: handle network errors
                  (when (exn? status) (ssh-log 'debug #:urgent status (exn-message status)))
                  (when block? (ssh-sendmail 'sendback status))
                  (if (ssh:msg:disconnect? packet)
    ...
    (with-handlers ([exn:ssh:eof? (lambda [[e : exn:ssh:eof]] (collapse (exn-message e) (exn:ssh:eof-reason e)) (raise e))]
      ...
      (parameterize ([current-custodian watchcat])
        ...
        (define identification : String (~a banner #:max-width 253))
        ...
        (let handshake : Void ()
          (match (sync/timeout/enable-break 3.14 /var/mail/send /var/mail/recv)
            [(? false?) (throw [exn:ssh:eof 'SSH_DISCONNECT_KEY_EXCHANGE_FAILED] "alogorithm negotiation timeout")]
