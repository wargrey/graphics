...
(bitmap-vl-append* #:gapsize 16
 (for/list ([family (in-list (list 'decorative 'roman 'script 'swiss 'modern 'symbol 'system))])
   ...
   (bitmap-vl-append (bitmap-text* (format "~a[~a]: Sphinx[#t]" (send font get-family) (send font get-face)) font)
                     (bitmap-text* (format "~a[~a]: Sphinx[#f]" (send font get-family) (send font get-face))
