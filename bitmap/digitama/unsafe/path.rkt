...
(module unsafe racket/base
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (define (bitmap_crawl! src key-nodes close? stroke fill-color dx dy fill-style density)
    (define cr (cairo_create src))
    (define-values (x1 y1 x2 y2) (cairo_path cr key-nodes close? dx dy fill-style density))
  ...
  (define (bitmap_crawl key-nodes close? flwidth flheight dx dy stroke fill-color fill-style density)
    ...
    (define-values (x1 y1 x2 y2) (cairo_path cr key-nodes close? (unsafe-fl+ dx inset) (unsafe-fl+ dy inset) fill-style density))

    (cairo-render cr stroke fill-color)
    (cairo_destroy cr)
    (values img (make-rectangular x1 y1) (make-rectangular x2 y2)))

  (define (cairo_path cr key-nodes close? dx dy fill-style density)
...
(unsafe-require/typed/provide
 (submod "." unsafe)
 [bitmap_crawl! (-> Bitmap-Surface (Listof (Pairof Char Float-Complex)) Boolean (Option Paint) (Option Bitmap-Source) Flonum Flonum Symbol Flonum
                    (Values Float-Complex Float-Complex))]
 [bitmap_crawl (-> (Listof (Pairof Char Float-Complex)) Boolean Flonum Flonum Flonum Flonum (Option Paint) (Option Bitmap-Source) Symbol Flonum
