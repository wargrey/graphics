...
(define plot-cartesian
  (lambda [#:id [id : (Option Symbol) #false]
           ...
           #:x-label [x-label : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) "x"]
           #:y-label [y-label : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) "y"]
           #:x-unit-desc [x-unit-desc : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) #false]
           #:y-unit-desc [y-unit-desc : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) #false]
           #:x-desc [x-desc : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) #false]
           #:y-desc [y-desc : (U Geo-Option-Rich-Text (Pairof Geo-Option-Rich-Text Geo-Option-Rich-Text)) #false]
    ...
    (define x-tick-layers : (Listof (GLayerof Geo))
      (append
       ...
       (for/fold ([labels : (Listof (GLayerof Geo)) null])
                 ([lbl (in-list (plot-axis-label-settings x-label x-unit-desc x-desc xaxis-min xaxis-max (if (or label-at-axis?) (* em 0.618) 0.0) 'x))])
         (if (car lbl)
             (let ([lbl.geo (plot-x-axis-label (car lbl) label-font label-color (caddr lbl) (cadddr lbl) desc-font desc-color (* em 0.382))])
               (case/eq (plot-axis-style-label-placement axis-style)
                 ...
                 [else (plot-axis-sticker-cons* lbl.geo (cadr (cdddr lbl)) (make-rectangular (cadr lbl) +0.0) 0.0+0.0i labels)]))
             labels))
         ...
         (if (car lbl)
             (let ([lbl.geo (plot-y-axis-label (car lbl) label-font label-color (caddr lbl) (cadddr lbl) desc-font desc-color)])
               (case/eq (plot-axis-style-label-placement axis-style)
                 ...
                 [else (plot-axis-sticker-cons* lbl.geo (cadr (cdddr lbl)) (flc-ri (- (cadr lbl))) Oview labels)]))
             labels))
