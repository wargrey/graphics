...
(define plot-grid-visual-values : (case-> [(Option Plot-Grid-Style) (U Index (-> Real Real Index)) (-> FlRGBA FlRGBA)
  ...
  (case-lambda
    [(self minor-count)
     (if (or self)
         (values (plot-grid-style-major-pen self)
                 (plot-grid-style-minor-pen self)
                 (or (plot-grid-style-minor-count self)
                     minor-count))
         (values #false #false minor-count))]
    [(self minor-count adjust)
     (if (or self)
         (let ([major-pen (plot-grid-style-major-pen self)]
               [minor-pen (plot-grid-style-minor-pen self)])
