...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define-type Plot-Fixed-Tick-Format (U False Plot-Tick-Format (Listof Geo-Option-Rich-Text)))
...
(define plot-fixed-ticks : (case-> [(Listof Real) -> Plot-Tick-Engine]
  ...
  (case-lambda
    [(ticks) (plot-fixed-ticks* ticks 0 plot-tick->label-text)]
    [(ticks labels) (plot-fixed-ticks* ticks 0 labels)]
    [(tmin tmax step) (plot-fixed-ticks* (range tmin (+ tmax step) step) 0 plot-tick->label-text)]
     ...
     (if (pair? ticks)
         (let ([rng (and #;(pair? (cdr ticks)) (cons (apply min ticks) (apply max ticks)))]
           ...
           (cond [(and (list? labels) (pair? (filter values labels)))
                  (let* ([lbls (for/hasheqv : (HashTable Real Geo-Option-Rich-Text) ([t (in-list ticks)]
                                                                                     [s (in-list labels)]
                                                                                     #:when s)
                                 (values t s))]
                         [predefine-labels (lambda [[tval : Real] [para : Integer]]
                                             (or (hash-ref lbls tval (λ [] #false))
                                                 (plot-tick->label-text tval para)))])
                 ...
                 [else (unsafe-plot-tick-engine layout plot-tick->label-text rng minor-count 1.0)]))
         plot-no-ticks)]
    [(ticks minor-count) (plot-fixed-ticks* ticks minor-count plot-tick->label-text)]
    [(tmin tmax step minor-count) (plot-fixed-ticks* (range tmin (+ tmax step) step) minor-count plot-tick->label-text)]
  ...
  (case-lambda
    ...
    [(ticks minor-count val.sym)
     ...
     (unsafe-plot-tick-engine (λ [tmin tmax] (values vals #false))
                              (plot-symbol-tick->label-text (cdr val.sym)
...
(define plot-interval-ticks : (->* (Real) ((Option Plot-Tick-Format) #:minor-count Plot-Minor-Tick-Count) Plot-Tick-Engine)
  (lambda [step [format #false] #:minor-count [minor-counts 0]]
    (unsafe-plot-tick-engine
     ...
     (or format plot-tick->label-text)
     #false
     minor-counts
     1.0)))
