  ...
  (lambda [#:rgb-filter [rgb-filter values]]
    ...
    (values (λ [L a b]
              (if (and (zero? a) (zero? b))
                  ...
                  (let* ([LMS (matrix* CIE-OKLAB->LMS-D65 (col-matrix [L a b]))]
                         [XYZ (matrix* CIE-LMS->XYZ-D65 (matrix-map (λ [[c : Flonum]] (* c c c)) LMS))]
                         [RGB (matrix* coXYZ XYZ)])
                    (define-values (R G B)
            ...
            (λ [R G B]
              (cond [(= R G B) (values (color-component-gamma-decode R) 0.0 0.0)]
                    [else (let-values ([(r g b) (color-gamma-decode R G B)])
                            ...
                            (values (~clamp (matrix-ref Lab 0 0) 0.0 1.0)
                                    (matrix-ref Lab 1 0)
                                    (matrix-ref Lab 2 0)))])))))
